...
- `npm run deploy` / `npm run deploy:dev` — builds, then `gcloud storage rsync` to `gs://clio.janelia.org` or `gs://clio-dev.janelia.org` and sets `Cache-Control: no-cache, max-age=0` on `index.html`. Requires the Google Cloud CLI (`gcloud`) on PATH.
